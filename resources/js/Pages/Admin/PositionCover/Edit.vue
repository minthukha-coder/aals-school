<template>
    <Layout>
        <h4 class="text-center">Edit Position Image</h4>
        <div class="row">
...
onMounted(() => {
    if (props.positionImage && props.positionImage.name) {
        formImageUrl.value = props.positionImage.name;
    }
});
...
const submit = () => {
    form.post(route('admin.position-images.update'), {
        forceFormData: true,
        onSuccess: () => {
            formImageUrl.value = null;
