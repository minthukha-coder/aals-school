<template>
    <Layout>
        <h4 class="text-center">Edit International Exam Course Cover</h4>
        <div class="row">
...
onMounted(() => {
    if (props.internationalCourseImage && props.internationalCourseImage.name) {
        formImageUrl.value = props.internationalCourseImage.name;
    }
});
...
const submit = () => {
    form.post(route('admin.international-course-images.update'), {
        forceFormData: true,
        onSuccess: () => {
            formImageUrl.value = null;
            toast.success('International Course Cover updated successfully!');
        },
        onError: (errors) => {
