<template>
    <Layout>
        <h4 class="text-center">Edit Cambridge Course Cover</h4>
        <div class="row">
...
onMounted(() => {
    if (props.cambridgeCourseImage && props.cambridgeCourseImage.name) {
        formImageUrl.value = props.cambridgeCourseImage.name;
    }
});
...
const submit = () => {
    form.post(route('admin.cambridge-course-images.update'), {
        forceFormData: true,
        onSuccess: () => {
            formImageUrl.value = null;
            toast.success('Cambridge Course Cover updated successfully!');
        },
        onError: (errors) => {
