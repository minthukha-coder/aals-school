<template>
    <Layout>
        <h4 class="text-center">Edit IGCSE Course Cover</h4>
        <div class="row">
...
onMounted(() => {
    if (props.igcseCourseImage && props.igcseCourseImage.name) {
        formImageUrl.value = props.igcseCourseImage.name;
    }
});
...
const submit = () => {
    form.post(route('admin.igcse-course-images.update'), {
        forceFormData: true,
        onSuccess: () => {
            formImageUrl.value = null;
            toast.success('IGCSE Cover updated successfully!');
        },
        onError: (errors) => {
