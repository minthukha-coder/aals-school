<template>
    <Layout>
        <h4 class="text-center">Edit Foundation Course Cover</h4>
        <div class="row">
...
onMounted(() => {
    if (props.foundationCourseImage && props.foundationCourseImage.name) {
        formImageUrl.value = props.foundationCourseImage.name;
    }
});
...
const submit = () => {
    form.post(route('admin.foundation-course-images.update'), {
        forceFormData: true,
        onSuccess: () => {
            formImageUrl.value = null;
            toast.success('Foundation Course updated successfully!');
        },
        onError: (errors) => {
