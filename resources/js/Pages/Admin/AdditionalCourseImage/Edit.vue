<template>
    <Layout>
        <h4 class="text-center">Edit Additional Course Cover</h4>
        <div class="row">
...
onMounted(() => {
    if (props.additionalCourseImage && props.additionalCourseImage.name) {
        formImageUrl.value = props.additionalCourseImage.name;
    }
});
...
const submit = () => {
    form.post(route('admin.additional-course-images.update'), {
        forceFormData: true,
        onSuccess: () => {
            formImageUrl.value = null;
            toast.success('Additional Course Cover updated successfully!');
        },
        onError: (errors) => {
